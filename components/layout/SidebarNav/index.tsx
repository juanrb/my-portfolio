		...
		<Tw.Container $open={open}>
			...
			<Tw.MenuItems>
				{menuItems.map((item, index) => (
					<Tw.MenuItem key={index} $spacing={item.spacing} $bottom={item.bottom}>
						<Link href={item.url} className='p-2 flex items-center'>
							<Tw.MenuItemIcon $active={isActive(item.url)}>{item.icon}</Tw.MenuItemIcon>
							<Tw.MenuItemTitle $active={isActive(item.url)} $open={open}>{item.title}</Tw.MenuItemTitle>
						</Link>
					</Tw.MenuItem>
				))}
			</Tw.MenuItems>
